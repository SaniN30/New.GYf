...
export default function HowItWorks() {
  return (
    <section id="how-it-works" className="py-32 bg-[#13111F]">
      <div className="max-w-5xl mx-auto px-6">
        <motion.div initial={{opacity:0,y:20}} whileInView={{opacity:1,y:0}} viewport={{once:true}} transition={{duration:0.7}}
          className="mb-20">
          <div className="text-xs font-mono text-purple-400 uppercase tracking-widest mb-4">Process</div>
          <h2 className="text-[clamp(2rem,5vw,4rem)] font-black text-white leading-tight">How GYF Works</h2>
        </motion.div>
        <div className="space-y-0">
          {steps.map((step, i) => (
            <motion.div key={step.num} initial={{opacity:0,x:-30}} whileInView={{opacity:1,x:0}} viewport={{once:true}} transition={{duration:0.7,delay:i*0.15}}
              className="flex gap-8 py-12 border-t border-white/5">
              <div className="text-[clamp(3rem,6vw,5rem)] font-black font-mono text-purple-500/20 leading-none flex-shrink-0 w-24">{step.num}</div>
              <div className="pt-2">
                <h3 className="text-xl font-bold text-white mb-3">{step.title}</h3>
                <p className="text-gray-400 leading-relaxed max-w-lg">{step.body}</p>
              </div>
            </motion.div>
          ))}
        </div>
