    ...
    <section className="py-32 px-6 bg-surface border-t border-border">
      <div className="max-w-7xl mx-auto">
        <motion.div variants={fadeIn} initial="hidden" whileInView="visible" viewport={viewportOnce} className="mb-16">
          ...
          <h2 className="text-text-primary font-light max-w-xl" style={{ fontFamily: "var(--font-cormorant)", fontSize: "clamp(2rem, 4vw, 3.5rem)", lineHeight: 1.1 }}>
            Four pillars of an AI stylist that earns trust.
          </h2>
        </motion.div>

        <motion.div variants={staggerContainer(0.15)} initial="hidden" whileInView="visible" viewport={viewportOnce} className="grid grid-cols-1 sm:grid-cols-2 gap-px bg-border">
          {pillars.map(({ icon: Icon, title, body }) => (
            <motion.div key={title} variants={fadeUp} className="card p-10 flex flex-col gap-5">
              <Icon size={20} className="text-accent" strokeWidth={1.5} />
              <h3 className="text-text-primary" style={{ fontFamily: "var(--font-cormorant)", fontSize: "clamp(1.25rem, 2vw, 1.75rem)", fontWeight: 500, lineHeight: 1.2 }}>
                {title}
              </h3>
